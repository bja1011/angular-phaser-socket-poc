...
export default class MinigameStartScene extends Phaser.Scene {
  ...
  generateDolphinTrashesOrder(isGift?) {
    ...
    for (let i = 1; i <= 6; i++) {
      this.bombReplacement(
        this.minimumTrashesAmount + 5,
        trashesOrderAmount - 2
      );
    }
    this.trashesId = this.trashesOrderArray.map((trash) => trash.id);
  }
  ...
  generateRandomTrashesArray(trashesObjAmount: number) {
    let previousIndex;
    do {
      const trashIndex = Phaser.Math.Between(
        0,
        this.SETTINGS.trashesDefinitions.length - 1
      );
  ...
  dolphinReplacement() {
    ...
    if (
      this.trashesOrderArray[randomNumber].type !== 'dolphin' &&
      this.trashesOrderArray[randomNumber - 1].type !== 'dolphin' &&
      this.trashesOrderArray[randomNumber + 1].type !== 'dolphin' &&
      this.trashesOrderArray[randomNumber].type !== 'gift'
    ) {
  ...
  bombReplacement(minIndex, maxIndex) {
    ...
    if (
      this.trashesOrderArray[randomNumber].type === 'dolphin' ||
      this.trashesOrderArray[randomNumber + 1].type === 'bomb' ||
      this.trashesOrderArray[randomNumber - 1].type === 'bomb' ||
      this.trashesOrderArray[randomNumber].type === 'gift'
    ) {
  ...
  giftReplacement(minIndex, maxIndex) {
    ...
    if (
      ...
      this.trashesOrderArray[randomNumber - 1].type !== 'dolphin' &&
      this.trashesOrderArray[randomNumber + 1].type !== 'dolphin' &&
      this.trashesOrderArray[randomNumber].type !== 'bomb'
    ) {
  ...
  createWithSettings(gameVersionNumber, minigameState?: MinigameState) {
    ...
    if (
      gameVersionNumber === MINIGAME_VERSION_STANDARD ||
      gameVersionNumber === MINIGAME_VERSION_GIFT
    ) {
  ...
  createBackground() {
    ...
    const bgGround = this.add.sprite(
      0,
      this.cameras.main.height - 18,
      MINIGAME_ATLAS,
      'sorting-ground-desktop.png'
    );
  ...
  createScoreBoard() {
    ...
    const ecoIcon = this.add.sprite(
      43,
      35,
      MINIGAME_ATLAS,
      'sorting-currency.png'
    );
  ...
  createLifeBoard() {
    ...
    for (let i = 1; i <= this.SETTINGS.maxLives; i++) {
      const life = this.add.sprite(
        lifeInitHorizontalValue,
        27,
        MINIGAME_ATLAS,
        'heart.png'
      );
  ...
  createCountdown() {
    this.countdownForeground = this.add.rectangle(
      0,
      0,
  ...
  checkTrashCollision() {
    if (this.trashElement) {
      for (const container of this.trashContainersArray) {
        ...
        if (
          this.trashElement.y >= container.y &&
          this.trashElement.y < container.y + 20 &&
          distance <= container.displayWidth / 2
        ) {
          container.animateContainerCollision();
          if (
            container.getData(TRASH_TYPE) ===
            TRASH_CONTAINERS_GAME_TYPES.DOLPHIN
          ) {
            this.createSplash();
          ...
          if (
            container.getData(TRASH_TYPE) === TRASH_CONTAINERS_GAME_TYPES.GIFT
          ) {
  ...
  createPoolContainer() {
    const poolContainer = new TrashContainer(
      this,
      134,
      this.cameras.main.height - 84,
      MINIGAME_ATLAS,
      TRASH_CONTAINERS_GAME_TYPES.DOLPHIN,
      'sorting-pool.png'
    );
  ...
  createGiftContainer() {
    const giftContainer = new TrashContainer(
      this,
      676,
      this.cameras.main.height - 104,
      MINIGAME_ATLAS,
      TRASH_CONTAINERS_GAME_TYPES.GIFT,
      'sorting-giftbox.png'
    );
  ...
  createDolphinBoard() {
    ...
    for (let i = 1; i <= this.SETTINGS.dolphinsAmount; i++) {
      const dolphin = this.add.sprite(
        dolphinInitHorizontalValue,
        25,
        MINIGAME_ATLAS,
        'dolphin-ia.png'
      );
  ...
  trashCrash() {
    ...
    const crash = this.add
      .sprite(crashX, crashY, 'crush')
